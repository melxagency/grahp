...
const supabase = createClient(const axios = require("axios");
const { createClient } = require("@supabase/supabase-js");
...
async function main() {
  ...
  // ⚠️ TOKEN DESDE GITHUB SECRET
  const token = process.env.PAGE_TOKEN;

  // 🔒 VALIDACIÓN CRÍTICA
  if (!token) {
    console.log("❌ ERROR: PAGE_TOKEN no está definido en GitHub Secrets");
    process.exit(1);
  }
  ...
  const startDate = new Date("2026-03-01"); // 👈 más antiguo fijo
